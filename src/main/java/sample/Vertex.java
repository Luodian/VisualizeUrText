...
/*
    ...
    persist instances of them or send them over a wire."
                                                         -"Effective Java"
 */
class Vertex<T> implements sample.VertexInterface<T>, java.io.Serializable {
    /**
    ...
    protected class Edge implements java.io.Serializable {
        private sample.VertexInterface<T> vertex;
        public Double weight;
	
	    protected Edge (sample.VertexInterface<T> _v, double _w) {
		    vertex = _v;
		    weight = _w;
	    }

        protected sample.VertexInterface<T> getEndVertex() {
            return vertex;
        }
    ...
    private sample.VertexInterface<T> previousVertex;
    private double cost;
    ...
    @Override
    public void addEdge (sample.VertexInterface<T> endVertex, double edgeWeight) {
        if (!this.equals(endVertex)) {
            ...
            while (!duplicate_Edges && edgeIterator.hasNext()) {
	            ...
	            sample.VertexInterface<T> neighbor_Vertex = nextEdge.getEndVertex ();
    ...
    @Override
    public void addEdge (sample.VertexInterface<T> endVertex) {
	    addEdge (endVertex, 0);
    }
    ...
    @Override
    public void setPredecessor (sample.VertexInterface<T> predecessor) {
	    this.previousVertex = predecessor;
    }

    @Override
    public sample.VertexInterface<T> getPredecessor() {
